...
const GlassCard = React.forwardRef<HTMLDivElement, GlassCardProps>(
  ({ className, title, subtitle, company, location, date, isEven, onDetailsClick, icon: Icon, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={`group h-[400px] w-full max-w-[400px] [perspective:1000px] ${className}`}
        {...props}
      >
        <div className={`relative h-full rounded-[40px] md:rounded-[50px] bg-gradient-to-br from-zinc-900 to-black shadow-2xl transition-all duration-500 ease-in-out [transform-style:preserve-3d] group-hover:[box-shadow:rgba(0,0,0,0.3)_30px_50px_25px_-40px,rgba(0,0,0,0.1)_0px_25px_30px_0px] ${hoverRotationClass}`}>
          <div className="absolute inset-2 rounded-[35px] md:rounded-[45px] border-b border-l border-white/10 bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-sm [transform-style:preserve-3d] [transform:translate3d(0,0,25px)]"></div>
          
          <div className="absolute inset-0 [transform:translate3d(0,0,26px)]">
            {/* Custom timeline content */}
            <div className="px-6 md:px-8 py-8 flex flex-col h-full">
              {date && (
          ...
          <div className="absolute bottom-5 left-8 right-8 flex items-center justify-between [transform-style:preserve-3d] [transform:translate3d(0,0,26px)]">
